...
public class Graph<T> {
	...
	public class Node {
		...
		public ArrayList<T> prefDistRec(double dist) {
			ArrayList<T> route = new ArrayList<T>();
			for (Edge i : this.neighbors) {
				Double newDist = dist - i.distCost;
				if (newDist > 0) {
					route.add(this.element);
					route.add(i.otherNode.element);
					route.addAll(i.otherNode.prefDistRec(newDist));
				}
			}
			return route;
		}
		
		public ArrayList<T> prefTimeRec(double time) {
			ArrayList<T> route = new ArrayList<T>();
			for (Edge i : this.neighbors) {
				...
				if (newTime > 0) {
					route.add(this.element);
					route.add(i.otherNode.element);
					route.addAll(i.otherNode.prefTimeRec(newTime));
				}
			}
			return route;
		}
	...
	public ArrayList<T> prefDist(T in, double dist) {
		...
		route = startNode.prefDistRec(dist);
		return route;
	}
	...
	public ArrayList<T> prefTime(T in, double time) {
		ArrayList<T> route = new ArrayList<T>();
		Node startNode = nodes.get(in);
		route = startNode.prefTimeRec(time);
		return route;
	}
}
